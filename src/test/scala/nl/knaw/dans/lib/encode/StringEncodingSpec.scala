...
class StringEncodingSpec extends FlatSpec with Matchers with OptionValues {
  ...
  "escapeString" should "return a string without any escaped characters when the input string contains only alphanumeric characters or a '_' character" in {
    val s = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_"
    s.escapeString shouldBe "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_"
  }
